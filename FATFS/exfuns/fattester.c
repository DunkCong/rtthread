...
u8 mf_read(u16 len)
{
	...
	for(i=0;i<len/512;i++)
	{
		res=f_read(file,my_buffer,512,&br);
		if(res)
		{
			printf("Read Error:%d\r\n",res);
			break;
		...
		{
			tlen+=br;
			for(t=0;t<br;t++)printf("%c",my_buffer[t]); 
		}
	}
	if(len%512)
	{
		res=f_read(file,my_buffer,len%512,&br);
		if(res)	//读数据出错了
		{
			printf("\r\nRead Error:%d\r\n",res);   
		}else
...
{
	return f_mkfs((const TCHAR*)path,mode,au,NULL,0);//格式化,drv:盘符;mode:模式;au:簇大小
} 
//删除文件/目录
//pname:文件/目录路径+名字
//返回值:执行结果
